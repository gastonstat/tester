...
Depending on your goals, \texttt{is.na()} might be enough. But what if we just want to simply test if a vector has missing values? With \texttt{tester} now we can with do that using the function \texttt{has\_missing()}:
<<is_na_ex2>>=
# test for missing values
